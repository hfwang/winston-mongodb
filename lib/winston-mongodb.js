...
var MongoDB = exports.MongoDB = function (options) {
  ...
  this.replicaSetName = options.replicaSetName || null;
  this.slaveOk = options.slaveOk || false;

  // Replica set is assumed to be in the form;
  // [ { host: 'host', port: port }, ... ]
  ...
  if (this.replicaSet) {
    var url = "mongodb://";
    for (var i = 0; i < this.replicaSet.length; i++) {
      url += this.replicaSet[i].host + ":" + this.replicaSet[i].port + ",";
    }
    url = url.substring(0, url.length - 1) + "/" + this.db + "?";
    if (this.replicaSetName) {
      url += 'replicaSet=' + this.replicaSetName + '&';
    }
    url += 'slaveOk=' + this.slaveOk + '&';
...
MongoDB.prototype.open = function (callback) {
  ...
  if (this.replicaSet) {
    ...
    this.client.open(err, function(err, mongoClient) {
      if (err) {
        throw err
      }
      self._ensureCollection(db, function () {
